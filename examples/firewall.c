...
Z3_ast mk_p_cnstrs(RS3_cfg_t cfg, RS3_packet_ast_t p1, RS3_packet_ast_t p2) {
  ...
  RS3_packet_extract_pf(cfg, p1, RS3_PF_IPV4_DST, &p1_l3_dst);

  RS3_packet_extract_pf(cfg, p2, RS3_PF_IPV4_SRC, &p2_l3_src);
  ...
  if (p1.key_id == p2.key_id) {
    Z3_ast _and_args[4] = { Z3_mk_eq(cfg->ctx, p1_l3_src, p2_l3_src),
    ...
    Z3_ast symmetric[4] = { Z3_mk_eq(cfg->ctx, p1_l3_src, p2_l3_dst),
...
int validate(RS3_cfg_t cfg, RS3_key_t k1, RS3_key_t k2) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    RS3_key_hash(cfg, k1, p12_1, &o12_1);
    RS3_key_hash(cfg, k2, p12_2, &o12_2);
