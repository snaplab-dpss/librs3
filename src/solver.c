...
void alarm_handler(int sig) {
  RS3_status_t status = RS3_STATUS_TIMEOUT;

  if (write(wp, &status, sizeof(RS3_status_t)) == -1) {
...
void p_ast_to_hash_input(RS3_cfg_t cfg, RS3_packet_ast_t p_ast,
...
void k_ast_to_rss_key(Z3_context ctx, Z3_ast k, RS3_key_t rssk) {
  /*
   *  I know what you're thinking. Getting the value of a bit vector
   *  by converting its string representation? Better call a priest
   *  and exorcise this code.
...
Z3_ast mk_hash_func(RS3_cfg_t cfg, unsigned iopt, Z3_ast p, Z3_ast key,
                    Z3_ast o) {
  Z3_ast k;
  Z3_ast p_and_k;
  Z3_ast o_bit;
...
Z3_ast pad_ast(RS3_cfg_t cfg, Z3_ast ast, unsigned new_size) {
...
Z3_ast mk_hash_eq(RS3_cfg_t cfg, Z3_ast *keys, RS3_packet_ast_t p1,
                  RS3_packet_ast_t p2) {
  Z3_ast k1;
  Z3_ast k2;
  Z3_ast p1_and_k1;
...
Z3_ast mk_d_const(RS3_cfg_t cfg, Z3_ast input, RS3_packet_t p) {
  ...
  RS3_status_t status;
  RS3_loaded_opt_t opt;

  unsigned offset, sz;
  unsigned input_sz;
  unsigned high, low;

  status = RS3_packet_to_loaded_opt(cfg, p, &opt);
  ...
  for (int ipf = RS3_FIRST_PF; ipf <= RS3_LAST_PF; ipf++) {
    ...
    for (unsigned byte = 0; byte < sz; byte++, field++) {
      ...
      and_args[offset + byte] =
          Z3_mk_eq(cfg->ctx, pf_const[offset + byte], pf_x[offset + byte]);
    }
...
RS3_status_t RS3_packet_extract_pf(RS3_cfg_t cfg, RS3_packet_ast_t p,
  ...
  if (input_sz != Z3_get_bv_sort_size(cfg->ctx, Z3_get_sort(cfg->ctx, p.ast)))
    return RS3_STATUS_PF_NOT_LOADED;

  status = RS3_loaded_opt_check_pf(p.loaded_opt, pf);

  if (status != RS3_STATUS_PF_LOADED)
    return status;

  for (int ipf = RS3_FIRST_PF; ipf <= RS3_LAST_PF; ipf++) {
    current_pf = (RS3_pf_t)ipf;
    status = RS3_loaded_opt_check_pf(p.loaded_opt, current_pf);

    if (status == RS3_STATUS_PF_UNKNOWN)
      return status;
    if (status != RS3_STATUS_PF_LOADED)
      continue;

    sz = RS3_pf_sz_bits(current_pf);
...
Z3_ast mk_key_byte_const(Z3_context ctx, Z3_ast key, unsigned byte,
                         RS3_byte_t value) {
  Z3_ast value_const;
  Z3_ast key_slice;
  Z3_sort byte_sort;
...
Z3_ast mk_key_const(Z3_context ctx, Z3_ast key, RS3_key_t k) {
...
Z3_ast mk_key_bit_const(Z3_context ctx, Z3_ast key, unsigned bit,
                        unsigned value) {
  Z3_ast key_slice;
  Z3_ast key_const;
  Z3_sort bit_sort;
 ...
 */
Z3_sort mk_d_sort(RS3_cfg_t cfg) {
  unsigned max_sz;
  Z3_sort d_sort;

  max_sz = RS3_cfg_max_in_sz(cfg);
  d_sort = Z3_mk_bv_sort(cfg->ctx, max_sz);

  return d_sort;
}

Z3_ast *mk_p(RS3_cfg_t cfg, Z3_ast d) {
  Z3_ast *p;
  Z3_sort d_sort;
...
bool is_equality_of_extracts(Z3_context ctx, Z3_ast ast) {
  ...
  Z3_decl_kind equals_lhs_func_kind =
      Z3_get_decl_kind(ctx, equals_lhs_func_decl);
  Z3_decl_kind equals_rhs_func_kind =
      Z3_get_decl_kind(ctx, equals_rhs_func_decl);
...
void sort_extract_equalities(Z3_context ctx,
  ...
  while (change) {
    ...
    for (unsigned i = 0; i < equalities->sz - 1; i++) {
      if (compare_extracts_top_idx(ctx, equalities->lhs[i],
                                   equalities->lhs[i + 1]) < 0) {
        Z3_ast tmp;
...
Z3_ast try_convert_extract_equalities_to_concat(Z3_context ctx, Z3_ast root) {
  ...
  for (unsigned i = 0; i < num_fields; i++) {
    ...
    if (is_equality_of_extracts(ctx, arg)) {
      ...
      updated_args[updated_args_sz] =
          try_convert_extract_equalities_to_concat(ctx, arg);
      updated_args_sz++;
    }
  }
...
Z3_ast mk_rss_stmt(RS3_cfg_t cfg, RS3_cnstrs_func mk_p_cnstrs, Z3_ast *keys) {
  Z3_sort d_sort;
  Z3_ast d1;
  Z3_ast d2;
  ...
  RS3_packet_ast_t p1_ast, p2_ast;
    ...
    for (unsigned k2 = 0; k2 < n_keys; k2++) {
      ...
      for (unsigned p1_iopt = 0; p1_iopt < n_loaded_opts; p1_iopt++) {
        ...
        for (unsigned p2_iopt = 0; p2_iopt < n_loaded_opts; p2_iopt++) {
          ...
          left_implies_and[0] = try_convert_extract_equalities_to_concat(
              cfg->ctx, left_implies_and[0]);
          left_implies_and[0] = Z3_simplify(cfg->ctx, left_implies_and[0]);

          if (wid == 0) {
            DEBUG_LOG("[Constraint info]\n");
            DEBUG_LOG("p1 option: %s\n",
                      RS3_opt_to_string(p1_ast.loaded_opt.opt));
            DEBUG_LOG("p1 device: %u\n", p1_ast.key_id);
            DEBUG_LOG("p2 option: %s\n",
                      RS3_opt_to_string(p2_ast.loaded_opt.opt));
          ...
          left_implies_and[1] =
              Z3_mk_not(cfg->ctx, Z3_mk_eq(cfg->ctx, p1_ast.ast, p2_ast.ast));

          left_implies = Z3_mk_and(cfg->ctx, 2, left_implies_and);
          right_implies = mk_hash_eq(cfg, keys, p1_ast, p2_ast);
          implies[n_implies] =
              Z3_mk_implies(cfg->ctx, left_implies, right_implies);

          n_implies++;

          assert(n_implies <= n_cnstrs && "Number of elements in implication "
                                          "must be <= number of constraints");
        }
      }
    }
  }
...
RS3_status_t RS3_packet_from_cnstrs(RS3_cfg_t cfg,
  ...
  RS3_key_hash_in_t hi2;

  RS3_status_t status;
  RS3_loaded_opt_t loaded_opt;
  RS3_packet_ast_t *shuffled_packet_ast;
  RS3_packet_ast_t p1_ast, p2_ast, p2_model_ast;

  status = RS3_packet_to_loaded_opt(cfg, data.packet_in, &loaded_opt);
  ...
  shuffled_packet_ast =
      (RS3_packet_ast_t *)malloc(sizeof(RS3_packet_ast_t) * cfg->n_loaded_opts);
  ...
  shuffle((void *)shuffled_packet_ast, cfg->n_loaded_opts,
          sizeof(RS3_packet_ast_t));
  ...
  hi2 = (RS3_key_hash_in_t)malloc(sizeof(RS3_byte_t) * loaded_opt.sz);

  p_ast_to_hash_input(cfg, p2_model_ast, hi2);

  *p_out = RS3_key_hash_in_to_packet(cfg, p2_model_ast.loaded_opt, hi2);
...
Z3_ast *assert_soft_constraints(Z3_context ctx, Z3_solver s,
                                unsigned num_cnstrs, Z3_ast *cnstrs) {
  unsigned i;
  Z3_ast *aux_vars;
...
void check_unsat_core(Z3_context ctx, Z3_solver s, unsigned num_soft_cnstrs,
                      Z3_ast *soft_cnstrs, bool *core_cnstrs) {
  Z3_ast *aux_vars =
      assert_soft_constraints(ctx, s, num_soft_cnstrs, soft_cnstrs);
  ...
  Z3_lbool is_sat =
      Z3_solver_check_assumptions(ctx, s, num_soft_cnstrs, assumptions);
...
void pseudo_partial_maxsat(Z3_context ctx, Z3_solver s, Z3_ast *keys,
                           RS3_key_t *keys_proposals, unsigned n_keys) {
  ...
  for (unsigned ikey = 0; ikey < n_keys; ikey++) {
    for (int bit = 0; bit < KEY_SIZE_BITS; bit++)
      key_constr[KEY_SIZE_BITS * ikey + bit] =
          mk_key_bit_const(ctx, keys[ikey], KEY_SIZE_BITS - 1 - bit,
                           BIT_FROM_KEY(bit, keys_proposals[ikey]));
  }
...
Z3_ast key_not_zero_cnstr(RS3_cfg_t cfg, Z3_ast key) {
  ...
  useful_bytes = RS3_cfg_max_in_sz(cfg) / 8 + 4;
  const_key_slices = (Z3_ast *)malloc(sizeof(Z3_ast) * (useful_bytes + 7));

  for (byte = 0; byte < useful_bytes - 1; byte++)
    const_key_slices[byte] =
        mk_key_byte_const(cfg->ctx, key, KEY_SIZE - byte - 1, 0);

  last_bits = 0;
  for (unsigned bit = byte * 8; bit < useful_bytes * 8 - 1; bit++) {
    const_key_slices[byte + last_bits] =
        mk_key_bit_const(cfg->ctx, key, KEY_SIZE_BITS - bit - 1, 0);
    last_bits++;
  }

  zero_key_bytes =
      Z3_mk_and(cfg->ctx, useful_bytes - 1 + last_bits, const_key_slices);
...
RS3_setup_t mk_setup(RS3_cfg_t cfg, RS3_cnstrs_func mk_p_cnstrs) {
  RS3_setup_t setup;
  Z3_sort key_sort;
  ...
  for (unsigned ikey = 0; ikey < cfg->n_keys; ikey++) {
    ...
    setup.keys_decl[ikey] =
        Z3_mk_func_decl(cfg->ctx, keys_symbol[ikey], 0, 0, key_sort);
...
RS3_status_t adjust_keys_to_cnstrs(RS3_cfg_t cfg, RS3_cnstrs_func mk_p_cnstrs,
                                   RS3_key_t *keys_seeds) {
  RS3_setup_t setup;
  Z3_model m;
  Z3_ast key_model;
...
RS3_status_t sat_checker(RS3_cfg_t cfg, RS3_cnstrs_func mk_p_cnstrs) {
  RS3_setup_t setup;
  ...
  return RS3_STATUS_HAS_SOLUTION;
}

void worker_key_adjuster(RS3_cfg_t cfg, RS3_cnstrs_func mk_p_cnstrs) {
  RS3_status_t status;
  RS3_key_t *keys;
  RS3_stats_t stats;

  keys = (RS3_key_t *)malloc(sizeof(RS3_key_t) * cfg->n_keys);
  ...
  for (unsigned ikey = 0; ikey < cfg->n_keys; ikey++) {
    RS3_key_rand(cfg, keys[ikey]);
    // RS3_key_only_ones(keys[ikey]);
  }

  status = adjust_keys_to_cnstrs(cfg, mk_p_cnstrs, keys);

  if (status == RS3_STATUS_NO_SOLUTION) {
    if (write(wp, &status, sizeof(RS3_status_t)) == -1) {
      DEBUG_PLOG("IO ERROR: unable to communicate status to manager\n");
    }
  ...
  for (unsigned ikey = 0; ikey < cfg->n_keys; ikey++) {
    ...
    if (!RS3_stats_eval(cfg, keys[ikey], &stats)) {
...
void worker_key_finder(RS3_cfg_t cfg, RS3_cnstrs_func mk_p_cnstrs) {
  RS3_status_t status;
  RS3_key_t *keys;
...
void worker_sat_checker(RS3_cfg_t cfg, RS3_cnstrs_func mk_p_cnstrs) {
  RS3_status_t status;
...
void launch_worker(RS3_worker worker, RS3_cfg_t cfg,
                   RS3_cnstrs_func mk_p_cnstrs, int p, comm_t comm) {
  int pid;
...
RS3_status_t master(RS3_cfg_t cfg, RS3_cnstrs_func mk_p_cnstrs, int np,
                    comm_t comm, RS3_key_t *keys) {
  RS3_status_t status;
  int wstatus;
  int maxfd;
  fd_set fds;
  ...
  for (;;) {
    ...
    for (int p = 0; p < np; p++) {
      ...
      if (read(comm.rpipe[p], &status, sizeof(RS3_status_t)) == -1) {
        ...
        case RS3_STATUS_SUCCESS: {
          for (unsigned ikey = 0; ikey < cfg->n_keys; ikey++) {
            if (read(comm.rpipe[p], keys[ikey], KEY_SIZE) == -1) {
              DEBUG_PLOG("IO ERROR: unable to read status from worker\n");
              continue; // FIXME: should I be reacting this way? Or should I
                        // retry?
            }
          }
